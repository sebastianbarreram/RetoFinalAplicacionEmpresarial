...
export class PlayerAPIService {
	...
	addPlayer(player: Player): Observable<Player> {
		return this.http.post<Player>(
			...
			tap(
				(newPlayer: Player) =>
					console.log(`added Player w/ id=${newPlayer.playerId}`),
			),
	...
	addPointsHistory(points: number, player: Player): Observable<any> {
		...
		return this.http.put(url, player, this.httpOptions).pipe(
			tap(
				(_) =>
					console.log(
						`addPointsHistory player id=${player.playerId} points=${player.pointsHistory}`,
					),
			),
	...
	addScore(score: number, player: Player): Observable<any> {
		...
		return this.http.put(url, player, this.httpOptions).pipe(
			tap(
				(_) =>
					console.log(
						`updated player id=${player.playerId} score=${player.score}`,
					),
			),
