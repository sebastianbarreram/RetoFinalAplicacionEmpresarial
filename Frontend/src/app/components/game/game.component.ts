...
export class GameComponent implements OnInit {
	...
	addPlayerInDb(card: Card): void {
		this.gameAPIService.addPlayerInGame(card.playerId).subscribe((game) => {
	...
	addCardInDb(card: Card): void {
		this.gameAPIService.addCardsInGame(card.cardId).subscribe((game) => {
	...
	iniciarJuego(): void {
		this.boardAPIService.getBoardById("62de01f1ee60c664c3d720fb").subscribe((
			...
			if (board.listCard.length !== (5 * board.idPlayers.length)) {
				this.cardAPIService.getRandomCards(
					board.idPlayers.length * 5,
				).subscribe((card) => {
					this.board.listCard.push(card);
	...
	timer(minute: number) {
		this.timerGame.setSeconds(minute);

		var seconds: number = minute;
		const prefix = minute < 10 ? "0" : "";

		this.timerGame.getObservable().subscribe(
			(data) => {
				seconds = Number(data);
				this.display = `${prefix}${Math.floor(seconds / 60)}:${seconds}`;
			},
			(error) => alert("Observer1, error code " + error),
			() => {
				if (!this.game.playerModelList.includes(this.playerId)) {
					const randomNumber = Math.floor(
						Math.random() * this.board.listCard.filter(
							(cardMap) => cardMap.playerId == this.playerId,
						).length,
					);
					const card = this.board.listCard.filter(
						(cardMap) => cardMap.playerId == this.playerId,
					)[randomNumber];
